...
class DoubleDQNAgent():
  ...
  def store_transition(self, state, action, reward, new_state, done):
    ...
    self.mem_cntr += 1
    self.priorities[index] = max(self.priorities) if (max(self.priorities)!=0 and max(self.priorities)!=np.nan) else 1
    self.priorities = np.nan_to_num(self.priorities, nan=0.8)

  ########## Priority Experience Buffer ##########
  def get_priorities(self, priority_scale):
    scaled_priorities = np.array(self.priorities) ** priority_scale
    sample_probabilities = scaled_priorities / sum(scaled_priorities)
    sample_probabilities = np.nan_to_num(sample_probabilities, nan=1)
    return sample_probabilities
  
  def get_importance(self, probabilities):
    importance = (1/self.mem_cntr) * (1/probabilities)
    for index in range(len(importance)):
      if np.isinf(importance[index]):
        importance[index] = 0
